...
- [[#other-packages][Other packages]]
  ...
  - [[#unreal-engine-compilation][Unreal Engine Compilation]]
  - [[#compilation-problems-panel][Compilation "Problems" Panel]]
- [[#todo][TODO]]
...
** Eldoc Box
Displays ElDoc documentation in a childframe.
The childframe is selectable and scrollable with mouse, even though the cursor is hidden.
...
** Javelin
Quick file and buffer bookmarking. Inspired by The Primaegans Harpoon.
Pin bookmarks to numbered positions (1-9) for instant access.
They are automatically separated by project and git branch.
...
Looks for a build.sh in the projectile projects root directory.
Also adds some sane customisations and defaults
...
Adds clang-style errors regex and other UE specific colouring options
...
** Compilation "Problems" Panel

Creates a buffer *Problems* backed by tabulated-list-mode
...
* TODO
- Creating a new window should take its size from all windows open it should rebalance
- Need an easy way to jump to header/cpp files
- Get clangd tags working and peeking definitions
- When doing a file search or a general text search we should be able to select previous entries
- Kill ring
- Auto format on save
- Vim hotkey for traversing camel case
- Magit
- Buffers with b b
- Files with f f?
